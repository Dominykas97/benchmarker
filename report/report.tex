...
\item[\texttt{prometheusUsesHttps}:] a Boolean variable indicating whether
  ...
  container uses HTTP.
\item[\texttt{dataDirectory}:] the directory where the control server saves
  performance data (should be configured as a persistent volume).
...
\begin{description}
\item[$\texttt{mode} \in \{ \text{startup}, \text{regular}, \text{both} \}$]
  determines when to simulate I/O: during the initialisation stage, with every
  call to \texttt{map()}, or in both situations.
...
The current state of compatibility with OpenShift can be described as follows:
\begin{itemize}
\item The simulation system can successfully run any number of experiments.
\item The control server can successfully query for and receive performance data
  from Prometheus.
\item Prometheus may or may not be able to record performance data, depending on
  the order in which various OpenShift components (e.g., pods, services, etc.)
  are launched and the delays between launching different components.
\end{itemize}
In order to make the OpenShift version of the system as easy to use as the
MiniShift version, one might want to develop a script similar to
\texttt{experiment.py}. The pseudocode of the script could look like this:
\begin{enumerate}
\item For all (relevant) resource limits:
  \begin{enumerate}
  \item Modify the manifesto of the TaskManager pod to impose these limits. (The
    manifestos for TaskManagers and JobManagers will have to be different.
    Furthermore, one might want to make the number of TaskManagers a
    configurable variable (within the execution script).)
  \item Run a series of experiments.
  \end{enumerate}
\item Retrieve the performance files from the persistent volume after the end of
  the \texttt{start} pod signals the end of the experiments.
\item Plot the data and save the plots. (Good plotting scripts are already
  developed with \texttt{ggplot2} and R, one just needs to integrate them with
  the rest of the experiment-running functionality.) \label{step:plotting}
\item Run the memory usage and CPU time validators on the data.
\item Generate a \LaTeX{} (or HTML...) report with plots, statistics about the
  experiments, and $p$-values generated by the statistical
  tests. \label{step:report}
\end{enumerate}
Obviously, Steps~\ref{step:plotting}--\ref{step:report} are (more or less)
optional and could be combined into a separate script instead.

...
\begin{algorithm}
  ...
  \For{$i \leftarrow 1$ \KwTo \numRequests}{
    \mySleep{\databaseLatency}\;
    $L \leftarrow$ \new \LinkedList of 64-bit integers\;
    \For{$j \leftarrow 1$ \KwTo
      $\left[\frac{\responseSize}{\nodeSize}\right]$}{
      add a random integer to $L$\;
      \mySleep{$\frac{\nodeSize}{\bandwidth}$}\;
    }
    \eIf{$\frac{\nodeSize}{\bandwidth} < \SI{15}{\milli\second}$}{
      \mySleep{$\frac{\responseSize}{\bandwidth}$}\;
    }{
      \mySleep{$\left\{ \frac{\responseSize}{\nodeSize} \right\} \times \frac{\nodeSize}{\bandwidth}$}\;
    }
    \If{$i < \numRequests$}{
      \mySleep{\intervalBetweenRequests}\;
    }
  }
  \Function{\mySleep{$t$}}{
  ...
  \label{alg:io}
\end{algorithm} % TODO: explain brackets, explain.

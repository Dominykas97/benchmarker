...
% TODO: we recommend running 20 tests per configuration
...
\begin{table}
  ...
  \begin{tabular}{c c c c}
    \toprule
    Our expectation & $\mu$ & Standard error & Statistical test passes \\
    \midrule
    \tablenum{64} & \tablenum{77.1} & \tablenum{1.3} & \xmark \\
    \tablenum{128} & \tablenum{128.1} & \tablenum{11.3} & \cmark \\
    \tablenum{256} & \tablenum{272.3} & \tablenum{5.8} & \xmark \\
    \tablenum{512} & \tablenum{514.9} & \tablenum{25.7} & \cmark \\
    \bottomrule
  \end{tabular}
  \label{tbl:gumbel}
\end{table}
...
While using a Gumbel distribution to model the data is a statistically sound
decision, defining the `acceptable' region by two standard errors is an
arbitrary choice and should be improved in the future, perhaps by returning a
$p$-value instead of a yes/no answer.

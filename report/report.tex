...
% TODO: outline the rest of the paper (fixing its structure as necessary)
...
% TODO: what are we doing with prometheus?
\begin{figure}
  \centering
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
  \caption{UML component diagram of the system. Yellow components are OpenShift
    manifestos, red components represent files (either Java classes or YAML
    configuration files), and cyan components are for everything else
    (components set up without explicit manifestos, etc.). Network connections
    are shown with ports and have port numbers (or application-layer protocol
    names) displayed.}
  \label{fig:uml}
\end{figure}
...
% TODO: information about DAGs should go somewhere here
...
constructs the DAG of components as described in \texttt{components.yaml}.
...
section, we introduce support for configurations of components expressed as a
DAG.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.32\textwidth}
    ...
    \begin{tikzpicture}[component/.style={draw,circle,fill}]
      ...
      \node[component,label=right:{Component 3}] at (0, -3) (component3) {};
    ...
    \end{tikzpicture}
    \caption{the topological structure imposed by the previous implementation}
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    \begin{tikzpicture}[component/.style={draw, circle, fill}]
      ...
      \draw[-{Stealth[length=2mm]}] (component2) -- (component3);
    \end{tikzpicture}
    \caption{an example of a DAG of components supported by the current
      implementation}
...
- parents:
  ...
  !\textcolor{verydarkgray}{<...>}!
\end{lstlisting}
    \caption{syntax for defining this DAG}
    \label{fig:topology_implementation}
  \end{subfigure}
  \caption{The topology of components: before and now}
\end{figure}

...
\newcommand{\boxit}[1]{\tikz[remember
  picture,overlay]{\node[yshift=3pt,fill=#1,opacity=.25,fit={(A)($(B)+(.39\linewidth,.8\baselineskip)$)}]
    {};}\ignorespaces}
...
\begin{algorithm}
  ...
  \For{$i \leftarrow 1$ \KwTo \numRequests}{
    ...
    \boxit{yellow}
    $t_1 \leftarrow \time{}$\;
    \mySleep{\textcolor{red}{$\frac{\responseSize}{\bandwidth} - t_1 + t_0$}}\;
    \If{$i < \numRequests$}{
      \mySleep{\textcolor{red}{\intervalBetweenRequests}}\;
    }
  }
  \Function{\mySleep{$t$}}{
  ...
  \label{alg:io}
\end{algorithm} % TODO: explain brackets, explain in general.
% responseSize/node_size is the number of nodes we want to have in the linked
% list. If we took no sleep breaks during the for loop, then we need to sleep at
% the end of it (since adding a random integer takes a negligible amount of
% time). Otherwise, if the linked list was supposed to have, say, 4.5 nodes, we
% end up with less memory usage, but we can still recover the fractional amount
% of running time that we're losing out on. We ignore sleeping instructions for
% less than 15 ms because they cannot be accurate due to properties of the
% underlying operating system [ref something]. Explain how we got
% responseSize/bandwidth (multiplying two things).
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../io_memory_tests/plots/prediction_density_relative.png}
    \caption{relative errors}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../io_memory_tests/plots/prediction_density_errors.png}
...
a standalone implementation of the algorithm by recording its running from
within the program itself. We explore all combinations of the following values:
...
% TODO: Replace all x's that are not part of a longer word with the numeric x. This is
% needed so that the 'x' in 'exp' is not replaced.
% The number of requests is then f(x_something) * binWidth. Then wait binWidth
% time before taking the next value.
% Flexible approach that allows us to model steady growth/decay, periodic
% patterns, and any number of spikes
